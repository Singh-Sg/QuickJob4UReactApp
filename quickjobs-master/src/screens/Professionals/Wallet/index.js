...
export default class WalletPro extends Component {
  constructor(props){
    super(props)
    this.state = {
      loading: true,
      ...
      AsyncStorage.getItem('user', (err, result) => {
        ...
        if(result !== null){
          ...
          postFormData(data, 'walletInfo', (response) => {
            if(response){
              this.setState({
                walletMoney: response.user_wallet_bls ? response.user_wallet_bls.toFixed(2) : 0,
          ...
          postFormData(data, 'getuserInfo', (response) => {
            if(response){
              onSignIn(response)
            }
          })
      ...
      return (
        <View style={styles.scrollViewContainer}>
          <View style={pageStyle.upgradedView}>
            ...
            <Button containerStyle={{paddingHorizontal: '20%', marginTop: 20}} style={{backgroundColor: '#fb8519', paddingVertical: 10}} onPress={() => {
                this.getWalletInfo()
                this.setState({
                  paymentConfirmed: false,
                  moneyValue: ''
      ...
      return <View style={pageStyle.popupView}>
        <TouchableOpacity style={pageStyle.closeIconView} onPress={() => this.closePopup()}>
          <View style={pageStyle.closeIconView}>
            <MaterialIcons name="clear" size={15} color="#fb8519" />
          </View>
        </TouchableOpacity>
        ...
        <View style={pageStyle.actionButtons}>
          <Button onPress={() => this.payNow()} loading={this.state.addMoneyLoading} style={{backgroundColor: '#fb8519', paddingVertical: 10}} textStyle={pageStyle.buttonText} text="PROCEED TO PAYMENT" />
        </View>
      </View>
    }
    return (
          ...
          <ScrollView showsVerticalScrollIndicator={false} style={pageStyle.listing}>
            <ImageBackground style={pageStyle.background} source={require('../../../assets/images/probg.jpg')}>
              <View style={pageStyle.textView}>
                ...
                <Text style={{color: '#fff', fontSize: 50, fontFamily: 'Proxima Nova'}}>{this.state.walletMoney}</Text>
              </View>
            </ImageBackground>
