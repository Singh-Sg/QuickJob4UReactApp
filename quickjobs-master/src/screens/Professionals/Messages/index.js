...
export default class MessagesPro extends Component {
  constructor(props){
    super(props)
    this.state = {
      loading: true,
  ...
  messagesData(item){
    if(item.sender_id == this.state.user.id){
      return <TouchableOpacity style={[pageStyle.listItem, {paddingVertical: 0, borderBottomWidth: 0}]} onPress={() => this.props.navigation.push('ChatsPro', {thread: item.id, name: item.Reciver.name, receiver_id: item.sender_id})}>
        <View style={pageStyle.listItem}>
          <View style={pageStyle.avatar}>
            <Image
              style={pageStyle.avatarImage}
              source={item.Reciver.image ? {uri: this.state.extraData.customer_image_url+item.Reciver.image} : require('../../../assets/images/male_avatar.png')}
            />
          </View>
    ...
    return <TouchableOpacity style={[pageStyle.listItem, {paddingVertical: 0, borderBottomWidth: 0}]} onPress={() => this.props.navigation.push('ChatsPro', {thread: item.id, name: item.Sender.name, receiver_id: item.sender_id})}>
      <View style={pageStyle.listItem}>
        <View style={pageStyle.avatar}>
          <Image
            style={pageStyle.avatarImage}
            source={item.Sender.image ? {uri: this.state.extraData.customer_image_url+item.Sender.image} : require('../../../assets/images/male_avatar.png')}
          />
        </View>
        <View style={pageStyle.content}>
    ...
    return (
      <SafeAreaView style={[styles.container, {backgroundColor: '#fb8519'}]}>
        ...
        {(!this.state.loading && this.state.messages.length > 0) && <View style={styles.container}>
          <ScrollView showsVerticalScrollIndicator={false} style={pageStyle.messagesListing}>
            <FlatList
              ref="listRef"
        ...
        {(!this.state.loading && this.state.messages.length == 0) && <View style={styles.loadingView}>
          <Text style={{fontSize: 20}}>No messages for you yet</Text>
        </View>}
      </SafeAreaView>
    );
  }
