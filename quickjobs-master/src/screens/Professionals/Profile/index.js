    ...
    ImagePicker.showImagePicker(options, (response) => {
      ...
      if (response.didCancel) {
        ...
        this.setState({
          ...
          coverImageURL: Platform.OS == 'android' ? response.path : ret
        }, () => {
  ...
  uploadCoverPicture(){
    ...
    if(this.state.coverImageURL){
      form_data.push({
        name: 'image',
        filename: 'file.jpeg',
        data: RNFetchBlob.wrap(this.state.coverImageURL),
        type: 'image/jpeg'
      })
    }
    RNFetchBlob.fetch('POST', 'https://quickjobs4u.com.au/api/home/changeCoverPic', { 
        'Content-Type' : 'multipart/form-data', 
    }, form_data).then((res) => {
      if(JSON.parse(res.data).code == 0){
        alert(JSON.parse(res.data).message)
