    ...
    return (
      <View style={styles.scrollViewContainer}>
       {!upgraded && <View style={styles.scrollViewContainer}>
        ...
        {activeTab == 2 && (user.professional_plan_id < 2 || exp_date < today || user.professional_plan_id == 2) && <Button containerStyle={pageStyle.submitButton} style={{backgroundColor: '#00c020', paddingVertical: 10}} loading={this.state.buttonLoading} onPress={() => ( exp_date < today) ? this.upgradePlan(2, extraData.Month_price_info.slice(0, -3)) : this.upgradePlanCheck(user.professional_plan_id ,exp_date)} text={exp_date < today ? "UPGRADE" : user.professional_plan_id == 2 ? "CURRENT" : "UPGRADE"} />}
